...
/// <summary>
/// Expression visitor for compiling shaped queries for the Markdown provider.
/// </summary>
public class MarkdownShapedQueryCompilingExpressionVisitor : ShapedQueryCompilingExpressionVisitor
{
    ...
    /// <summary>
    /// Creates a new instance of <see cref="MarkdownShapedQueryCompilingExpressionVisitor"/>.
    /// </summary>
    public MarkdownShapedQueryCompilingExpressionVisitor(
    ...
    protected override Expression VisitShapedQuery(ShapedQueryExpression shapedQueryExpression)
    {
        ...
        if (queryExpression is MarkdownQueryExpression markdownQueryExpression)
        {
            var entityType = markdownQueryExpression.EntityType;

            // Create the enumerable expression that reads from the markdown store
    ...
    {
        // Get the method info for the query enumerable
        var queryMethod = typeof(MarkdownShapedQueryCompilingExpressionVisitor)
            .GetMethod(nameof(QueryEntities), BindingFlags.NonPublic | BindingFlags.Static)!
            .MakeGenericMethod(entityType.ClrType);

        // Create expression to call the query method
    ...
    private static IEnumerable<TEntity> QueryEntities<TEntity>(QueryContext queryContext, IEntityType entityType)
        where TEntity : class
    {
        var database = queryContext.Context.GetService<IMarkdownDatabase>();
        var store = database.Store;
        var table = store.GetTable(entityType);

        // Get or create cached materializer for this entity type
    ...
    /// <summary>
    /// Creates a compiled materializer for the entity type with cached property setters.
    /// </summary>
    ...
    /// <summary>
    /// Creates a compiled property setter delegate for fast property assignment.
    /// </summary>
    private static Action<object, object?> CreatePropertySetter(Type entityType, PropertyInfo propertyInfo)
    {
        ...
        // Cast instance to the entity type
        var castInstance = Expression.Convert(instanceParam, entityType);

        // Cast value to the property type (handle nullable types)
    ...
    /// <summary>
    /// Cached entity materializer with compiled property setters.
    /// </summary>
